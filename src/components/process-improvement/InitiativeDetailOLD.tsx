...
export function InitiativeDetailOLD({
  ...
  const [selectedTaskForFiles, setSelectedTaskForFiles] = useState<TaskOLD | null>(null);

  const handleCreateTask = (taskData: Omit<TaskOLD, 'id'>) => {
    const newTask: TaskOLD = {
      ...taskData,
      id: `t${Date.now()}`,
    };
  ...
  const handleEditTask = (task: TaskOLD) => {
    setEditingTask(task);
